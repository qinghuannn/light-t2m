...
def bi_mamba_inner_ref(
    xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
    out_proj_weight, out_proj_bias,
    A, A_b, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
    C_proj_bias=None, delta_softplus=True, text_len=None
):
    ...
    if text_len is not None:
        nx = torch.cat([x[..., :text_len], x[..., text_len:].flip([-1])], dim=-1)
    else:
        nx = x.flip([-1])
    y_b = selective_scan_fn(nx, delta.flip([-1]), A, B.flip([-1]), C.flip([-1]), D, z=z.flip([-1]), delta_bias=delta_bias, delta_softplus=True)
    y = (y + y_b) / 2
